...
const rootReducer  = (state = defaultState, action:any) => {
	...
	if(type === CREATE_BPM_ACTION_TABS){
		...
		return newState;
	} else if(type === PROCESS_TRACES_UI_OPEN ) { // 打开流程办理界面
		const newState = JSON.parse(JSON.stringify(state));
		newState.BpmWaitingProcessState.switchProcessTrace = true
		return newState;
	} else if(type === PROCESS_TRACES_UI_CLOSE) { // 关闭流程办理界面
		const newState = JSON.parse(JSON.stringify(state));
	...
	} else if(type === PROCESS_INCREASE_SIGN_UI_ClOSE) {
		
	} else if(type === PROCESS_REDUCE_SIGN_UI_OPEN) {

	} else if(type === PROCESS_REDUCE_SIGN_UI_CLOSE) {
